    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex align-items-center justify-content-between w-100 flex-wrap gap-3">
                <div class="input-icon">
                    ...
                    <input type="text" id="dt-search" class="form-control" style="width: 260px;" placeholder="Search blocked users..." />
                </div>
        ...
        <div class="card-body">
            ...
            @include('admin.layouts.partials._table-empty', [
                ...
                'subtitle' => 'There are currently no blocked users in the system.'
            ])
        </div>
    </div>
    ...
    <script>
        $(function () {
            let table = $('#users-table').DataTable({
                serverSide: true,
                processing: false,
                ajax: '{{ route('admin.users.blocked.data') }}',
                columns: [
            ...
            $(document).on('click', '.btn-unblock', function () {
                ...
                Swal.fire({
                    ...
                    $.post('/admin/users/' + id + '/unblock', { _token: '{{ csrf_token() }}' })
                        .done(function () {
                            table.ajax.reload(null, false);
                            Swal.fire({ toast: true, position: 'top', showConfirmButton: false, timer: 1500, icon: 'success', title: 'User unblocked' });
                        })
                        .fail(function () {
