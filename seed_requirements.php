...
echo "Generating genuine requirements...\n";
...
foreach ($childCategories as $catId => $catName) {
    ...
    for ($i = 0; $i < 5; $i++) {
        ...
        $budget = rand(500, 25000);
        $quantity = rand(1, 50);
        
        $description = "I am looking for $productName in good condition. My budget is around $budget. Please contact me if you have this available in or around Gurugram. Looking to close the deal as soon as possible.";
        ...
        $requirementsToInsert[] = [
            'category_id' => $catId,
            'title' => "Required: " . $productName,
            'description' => $description,
            'quantity' => $quantity,
            'target_budget' => $budget,
            'delivery_location' => $city . ', India',
            'delivery_pincode' => '122001',
            'city' => $city,
            'latitude' => $lat,
            'longitude' => $lng,
            'user_id' => $randomUserId,
            'status' => \App\Models\Requirement::STATUS_OPEN,
...
foreach ($requirementsToInsert as $data) {
    \App\Models\Requirement::create($data);
}

echo "Successfully seeded " . count($requirementsToInsert) . " requirements across all subcategories!\n";
